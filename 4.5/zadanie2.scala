...
object SalaryCalculator {
  def calculateMonthlySalary(annualIncome: Double, bonusPercent: Double, foodCompensation: Double): Double = {
  ...
  def main(args: Array[String]): Unit = {
    val annualIncome = readDouble("Введите годовой доход сотрудника до вычета налогов: ")
    val bonusPercent = readDouble("Введите процент премии к годовому доходу: ")
    val foodCompensation = readDouble("Введите сумму компенсации за питание: ")

    val monthlySalary = calculateMonthlySalary(annualIncome, bonusPercent, foodCompensation)
...
object SalaryDeviationCalculator {
  ...
  val salaries = List(100000, 150000, 200000, 80000, 120000, 75000, SalaryCalculator.calculateMonthlySalary(100000, 10, 2000))
...
object AdjustedSalaryCalculator {
  ...
  def calculateAdjustedSalary(monthlySalary: Double, lastDeviation: Double): Double = {
    if (lastDeviation > 0) monthlySalary + 20000
    else monthlySalary - 10000
  }

  def main(args: Array[String]): Unit = {
    val adjustedMonthlySalary = calculateAdjustedSalary(SalaryDeviationCalculator.salaries.last, SalaryDeviationCalculator.percentageDeviations.last)
    val updatedSalaries = SalaryDeviationCalculator.salaries.dropRight(1) :+ adjustedMonthlySalary
...
object SalarySortedList {
  ...
  val updatedSalaries = SalaryDeviationCalculator.salaries ++ newEmployeesSalaries
...
object EmployeeRanking {
  import SalarySortedList._

  val updatedRanking: List[(Int, Double)] = {
    val newEmployeeSalary = 130000
    val newUpdatedSalaries: List[Double] = sortedSalaries :+ newEmployeeSalary
...
object MiddleLevelEmployees {
  ...
  val minMiddleSalary: Double = readDouble("Введите минимальную зарплату специалиста уровня middle: ")
  val maxMiddleSalary: Double = readDouble("Введите максимальную зарплату специалиста уровня middle: ")
